...
from model.train_utils import MaskMol, fix_train_random_seed, save_finetune_ckpt, metric_reg
from dataloader.image_dataloader import train_test_val_idx, smiles_label, ImageDataset
...
def evaluate(model, data_loader, criterion, device, epoch, return_data_dict=False):
    ...
    if return_data_dict:
        ...
        return accu_loss.item() / (step + 1), metric_reg(y_true, y_scores), data_dict
    else:
...
def main(args):
    ...
    train_idx, test_idx, val_idx = train_test_val_idx(args.dataroot, args.dataset)

    train_smi, train_label, test_smi, test_label, val_smi, val_label = smiles_label(args.dataroot, args.dataset,
        ...
        if is_left_better_right(valid_result, results['highest_valid'], standard=valid_select):
            ...
            if args.save_finetune_ckpt == 1:
                save_finetune_ckpt(args.resume, model, optimizer, round(train_loss, 4), epoch, args.ckpt_dir,
                                   "valid_best", lr_scheduler=None, result_dict=results)
            early_stop = 0
        else:
            early_stop += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_metric', type=str, default="mae", help='eval metric mae rmse mse')
